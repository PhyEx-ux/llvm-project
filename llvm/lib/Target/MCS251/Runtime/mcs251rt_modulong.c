 ...
 */
/*
 * MCS251 运行时：32 位无符号取模。链接符号 __modulong；叶函数（零外部
 * 调用——与除法助手互不调用，div 与 rem 同现为两次独立 libcall，
 * §10-Q6）；第二参数槽 __modulong_PARM_2 = 4B（大端）。
 *
...
/*
 * 恢复余数取模（移位-比较），32 轮，每轮至多移 1 位，循环定数必然终止。
 * 与 mcs251rt_divulong.c 同一循环结构，只保留部分余数、不累计商。
 *
