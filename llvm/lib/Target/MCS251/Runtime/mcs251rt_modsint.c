 ...
 */
/*
 * MCS251 运行时：16 位有符号取模。链接符号 __modsint；非叶（恰有一次
 * 对 __moduint 的显式调用）；第二参数槽 __modsint_PARM_2 = 2B（大端）。
 *
 ...
 * （C11 6.3.1.3p3 实现定义），非可移植性质。代表位型：余 -1 为 0xFFFF。
 * 合法有符号余数满足 |r| < |b| <= 2^15，取不到 -32768（INT16_MIN），
 * 故不以其为例（复审修正 2026-09-07）。工具链身份或 §6.1 第 1-4 项任一
 * 编译条件变化即触发该依赖的全量重验。
 *
