 ...
 */
/*
 * MCS251 运行时：16 位无符号除法。链接符号 __divuint；叶函数（零外部
 * 调用）；第二参数槽 __divuint_PARM_2 = 2B（大端）。
 *
...
uint16_t _divuint(uint16_t x, uint16_t y)
{
    ...
    for (i = (uint32_t)0u; i < (uint32_t)16u; ++i) {
        ...
        if (r >= y) {
            r = t;
            q = (uint16_t)(((unsigned)q << 1) | 1u);
        } else {
            q = (uint16_t)((unsigned)q << 1);
        }
    }
    return q;
}
