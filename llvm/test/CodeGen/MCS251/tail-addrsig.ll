; RUN: llc -mtriple=mcs251 -verify-machineinstrs -O0 -addrsig < %s | FileCheck %s --implicit-check-not=.addrsig
; RUN: llc -mtriple=mcs251 -verify-machineinstrs -O2 -addrsig < %s > %t.asm
; RUN: FileCheck %s --implicit-check-not=.addrsig < %t.asm
; RUN: llc -mtriple=mcs251 -O2 -addrsig=false < %s > %t.no-addrsig.asm
; RUN: diff %t.asm %t.no-addrsig.asm
; RUN: llc -mtriple=mcs251 -verify-machineinstrs -O2 -filetype=obj -addrsig < %s > %t.rel
; RUN: llc -mtriple=mcs251 -O2 -filetype=obj -addrsig=false < %s > %t.no-addrsig.rel
