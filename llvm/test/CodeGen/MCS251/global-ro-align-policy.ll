; RUN: split-file %s %t
; RUN: not --crash llc -mtriple=mcs251 %t/ptrtbl.ll -o - 2>&1 | FileCheck %s --check-prefix=RO
; RUN: not --crash llc -mtriple=mcs251 -filetype=obj %t/ptrtbl.ll -o - 2>&1 | FileCheck %s --check-prefix=RO
...
; stay loud: initializer relocations (pointer tables), mutable aligned
; storage (byte-alignment is still required in DSEG) and custom sections
; are rejected with their established messages.
