...
@_far = external global i8
@_slot = external global ptr

define i32 @_pointer_size() {
...
define ptr @_global_carry() {
; CHECK-LABEL: _global_carry:
; CHECK: (_far+32) >> 8, (_far+32)
; CHECK: (_far+32) >> 16
  %p = getelementptr i8, ptr @_far, i32 32
  ret ptr %p
}
define i8 @_ptr_inc(ptr %p) {
...
define i8 @_ptr_roundtrip(ptr %p) {
; CHECK-LABEL: _ptr_roundtrip:
; CHECK: .db 0x7e,
; CHECK: mov @dr{{[0-9]+}}
; CHECK: mov {{r[0-9]+}}, @dr{{[0-9]+}}
  store volatile ptr %p, ptr @_slot, align 1
  %q = load volatile ptr, ptr @_slot, align 1
  %same = icmp eq ptr %p, %q
  %r = zext i1 %same to i8
  ret i8 %r
}
define i32 @_load32(ptr %p) {
; CHECK-LABEL: _load32:
; CHECK: mov {{r[0-9]+}}, @dr{{[0-9]+}}
...
; CHECK: ecall _stack_reader
  ...
  %v = call i8 @_stack_reader(ptr %p)
  ret i8 %v
}
define i8 @_stack_reader(ptr %p) {
