...
; V05.50.4):
;   * @wr      : register-indirect, region-00 edata (a runtime pointer)
;   * @wr+dis16: displaced; a global folds its offset into `mov wr,#(_sym+off)`
;   * dir8     : direct, constant address <= 0xff (0x00-0x7f page-zero edata,
;                0x80-0xff SFR -- the SFRs are ONLY reachable this way; the
;                same numeric address via @wr is region-00 edata)
...
; the ONLY encoding that does: `mov wr,#0x99; mov r,@wr` would address
...
; Constant address above the direct range: materialise + @wr.
...
; CHECK:         mov [[HI:r[0-9]+]], @wr{{[0-9]+}}+0x0000
; CHECK-NEXT:    mov [[LO:r[0-9]+]], @wr{{[0-9]+}}+0x0001
...
; CHECK-NEXT:    mov [[HI:r[0-9]+]], @wr{{[0-9]+}}+0x0000
; CHECK-NEXT:    mov [[LO:r[0-9]+]], @wr{{[0-9]+}}+0x0001
; CHECK-NEXT:    mov dpl, [[LO]]
; CHECK-NEXT:    mov dph, [[HI]]
  %v = load i16, ptr @gv16
...
; CHECK:         mov [[HI:r[0-9]+]], @wr{{[0-9]+}}+0x0001
; CHECK-NEXT:    mov [[LO:r[0-9]+]], @wr{{[0-9]+}}+0x0002
; CHECK-NEXT:    mov dpl, [[LO]]
; CHECK-NEXT:    mov dph, [[HI]]
  %q = getelementptr i8, ptr %p, i16 1
  %v = load i16, ptr %q
