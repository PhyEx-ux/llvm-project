/*
 * Explicit big initializer blocks for the X4 e2e sources (generated once,
 * frozen as a source asset; the byte values are the runtime expectations
 * too).  Patterns are deliberately non-trivial (no runs, no symmetry).
 * Every body line ends with a trailing comma (legal in initializers).
 */
#ifndef XDATA_E2E_INITBYTES_H
#define XDATA_E2E_INITBYTES_H

/* BUF256[i] = (i * 5 + 1) & 0xff */
#define XDATA_E2E_BUF256_INIT { \
...
/* EXT16[i] = (0xA0 + i * 3) & 0xff */
#define XDATA_E2E_EXT16_INIT { \
    0xA0, 0xA3, 0xA6, 0xA9, 0xAC, 0xAF, 0xB2, 0xB5, 0xB8, 0xBB, 0xBE, 0xC1, 0xC4, 0xC7, 0xCA, 0xCD, \
}

/* MOD32[i] = (i * 9 + 7) & 0xff */
#define XDATA_E2E_MOD32_INIT { \
    0x07, 0x10, 0x19, 0x22, 0x2B, 0x34, 0x3D, 0x46, 0x4F, 0x58, 0x61, 0x6A, 0x73, 0x7C, 0x85, 0x8E, \
    0x97, 0xA0, 0xA9, 0xB2, 0xBB, 0xC4, 0xCD, 0xD6, 0xDF, 0xE8, 0xF1, 0xFA, 0x03, 0x0C, 0x15, 0x1E, \
}

/* FW pattern: FWBUF64[i] = (i * 5 + 1) & 0xff (BUF256's first 64 values) */
#define XDATA_E2E_FWBUF64_INIT { \
    0x01, 0x06, 0x0B, 0x10, 0x15, 0x1A, 0x1F, 0x24, 0x29, 0x2E, 0x33, 0x38, 0x3D, 0x42, 0x47, 0x4C, \
    0x51, 0x56, 0x5B, 0x60, 0x65, 0x6A, 0x6F, 0x74, 0x79, 0x7E, 0x83, 0x88, 0x8D, 0x92, 0x97, 0x9C, \
    0xA1, 0xA6, 0xAB, 0xB0, 0xB5, 0xBA, 0xBF, 0xC4, 0xC9, 0xCE, 0xD3, 0xD8, 0xDD, 0xE2, 0xE7, 0xEC, \
    0xF1, 0xF6, 0xFB, 0x00, 0x05, 0x0A, 0x0F, 0x14, 0x19, 0x1E, 0x23, 0x28, 0x2D, 0x32, 0x37, 0x3C, \
...
/* FW cross-TU: FWEXT16[i] = (0x5C + i * 7) & 0xff */
#define XDATA_E2E_FWEXT16_INIT { \
    0x5C, 0x63, 0x6A, 0x71, 0x78, 0x7F, 0x86, 0x8D, 0x94, 0x9B, 0xA2, 0xA9, 0xB0, 0xB7, 0xBE, 0xC5, \
}

/* MOD cross-TU: MODEXT16[i] = (0xC3 - i) & 0xff */
#define XDATA_E2E_MODEXT16_INIT { \
