...
高置信可观察性假说；当前真机版改为低速重发状态。

**最终selftest真机闭环已达成**：用户经PM回报，当前发布
`/mnt/c/Prj/LLVM/realhw-hex/selftest.hex` 稳定循环输出 `SELFTEST-PASS`，观察间隔
约330ms。SHA256为 `36f15a97ac167a32743ee330525a486ece5a3f3dd6fc96b8a9c929b21339169f`。
此前乱码插曲确认为串口助手显示设置问题，没有为此修改生产发送代码；
`selftest-v1tx/gap`诊断脚本未执行构建/发布即取消。约330ms不是精确周期承诺。
新板复验仍应终端就绪后复位并保存全部检查项，不把最终状态行当作原始日志替代。
