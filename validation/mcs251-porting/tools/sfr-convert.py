...
only.  XFR registers (0xFE00..0xFFFF / 0x7EF000..0x7EFFFF) are not
representable in AS6 and stay comment-only registrations in both modes; the
same holds for ``sbit`` aliases.
"""
...
def render_header(
    source: Path, output: Path, c: Classification, address_space: int = 0
) -> str:
    ...
    # XFR registers live in the 0xFE00..0xFFFF / 0x7EF000..0x7EFFFF windows,
    # not in the 8-bit direct space (0x80..0xFE) that address space 6 denotes,
    # so the AS6 mode cannot apply to them; registrations stay comment-only.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "moves under the v2 memory contract. XFR and sbit registrations "
            "are unaffected: AS6 only denotes the 8-bit direct window."
        ),
    )
    ...
    output.write_text(
        render_header(source, output, classification, args.sfr_address_space),
        encoding="utf-8",
        newline="\n",
    )
    report.write_text(
        json.dumps(
            report_object(source, output, classification, args.sfr_address_space),
            indent=2,
            ensure_ascii=False,
        )
        + "\n",
