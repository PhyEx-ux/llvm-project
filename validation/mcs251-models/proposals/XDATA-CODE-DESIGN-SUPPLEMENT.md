...
链接种裁：目的区间 `[bank:window, +object_size)` 必须 24 位内、单 64K 窗、完整落入一个已分配 XSEG 区间（含末字节）、记录间目的不得重叠；节名匹配 `.mcs251.xdata_init` 或 `.mcs251.xdata_init.` 前缀。链接器不合成 XDATA_INIT 字节，唯一例外（G13b，D4 裁定）：对携带 `SHF_MCS251_XSEG_SPLIT` 的输入 XSEG 节，链接器按窗切分合成 clear-only v1 记录并写入链接器拥有的 XDATA_INIT 输出；每条合成记录与输入记录接受同一套结构性、单 64K 窗、XSEG 包含与不重叠校验（CRT 消费循环属 X4）。

### 7.4 XSEG 单对象 65535 上限、64K 跳 bank、洞计入 l_XSEG

- 单对象上限 65535 字节（无标志节，现状不变）：u16 记录字段所限，且无标志 XSEG 对象不跨 64K 窗（单 bank 记录描述不了跨界对象）；越限 = 硬错误。
- 带标志逻辑大对象（G13b：v2 对象携带 `SHF_MCS251_XSEG_SPLIT`、全零初值）可跨窗：链接器将其单区间连续放置（跳过 65535 上限门与跳 bank），再按窗切分为多条单窗 clear-only v1 记录；7.3 的记录种裁对合成记录原样生效。
- 顺序分配不跨界（无标志节）：游标到窗口末尾剩余不足整节时，跳到下一 bank 起点（64K 对齐）再分配；带标志节跳过该分支，游标按节末跨窗推进。
- 跳 bank 留下的洞计入 `l_XSEG` 跨度（`l_XSEG` = 最高已分配字节上界 − `s_XSEG`），不进 DSEG、不动栈高水位；同样计入 `--xdata-size` 容量（7.2）。
- 双保险（无标志节）：手造跨窗的 xdata_init 记录、显式跨窗的逐节起点，均为硬错误；带标志节的显式跨窗起点按字面执行（跨窗是其本意，I2.6 豁免），仍受 24 位 rangeFits、重叠门与 `--xdata-size` 容量门约束。
