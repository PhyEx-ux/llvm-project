...
W=/home/liu/mcs251-demo-test-t2/build/t2-gpio-readback
rm -rf "$W"
mkdir -p "$W"
...
# Inspect assembly before accepting the object path. These are all direct SFR
# operands (0x80/0x90 ports and 0x91-0x94 mode registers).
"$BIN/llc" -mtriple=mcs251-unknown-none -filetype=asm -o "$W/gpio-readback.asm" "$CASE/gpio-readback.ll"
for addr in 80 90 91 92 93 94; do
  if ! grep -Eiq "(^|[^0-9a-f])0x?$addr([^0-9a-f]|$)" "$W/gpio-readback.asm"; then
    echo "gpio-readback: missing direct SFR operand $addr" >&2
    exit 1
  fi
done
"$BIN/llc" -mtriple=mcs251-unknown-none -filetype=obj -o "$W/gpio-readback.rel" "$CASE/gpio-readback.ll"
: > "$W/gpio-readback.lst"
...
printf '%s\n' \
  ...
  '-b GSINIT0 = 0xfc2800' \
  "$W/crt0.rel" "$W/harness.rel" "$W/gpio-readback.rel" \
  '-e' > "$W/image.lk"
...
if [ ! -f "$W/image.hex" ]; then
  echo "gpio-readback: linker did not produce image.hex" >&2
  exit 1
fi
set +e
...
PY
if [ "$QRC" -ne 124 ]; then
  echo "gpio-readback: expected timeout 124, got $QRC" >&2
  exit 1
fi
printf 'gpio-readback: PASS\n'
