...
W=/home/liu/mcs251-demo-test-t2/build/t2-uart-echo-poll
rm -rf "$W"
mkdir -p "$W"
...
# First emit assembly and inspect the low-level SCON/SBUF direct operands.
"$BIN/llc" -mtriple=mcs251-unknown-none -filetype=asm -o "$W/uart-echo-poll.asm" "$CASE/uart-echo-poll.ll"
for addr in 98 99; do
  if ! grep -Eiq "(^|[^0-9a-f])0x?$addr([^0-9a-f]|$)" "$W/uart-echo-poll.asm"; then
    echo "uart-echo-poll: missing direct SFR operand $addr" >&2
    exit 1
  fi
done
"$BIN/llc" -mtriple=mcs251-unknown-none -filetype=obj -o "$W/uart-echo-poll.rel" "$CASE/uart-echo-poll.ll"
: > "$W/uart-echo-poll.lst"
...
printf '%s\n' \
  ...
  '-b GSINIT0 = 0xfc2800' \
  "$W/crt0.rel" "$W/harness.rel" "$W/uart-echo-poll.rel" \
  '-e' > "$W/image.lk"
...
if [ ! -f "$W/image.hex" ]; then
  echo "uart-echo-poll: linker did not produce image.hex" >&2
  exit 1
fi

set +e
# Keep the injected byte on stdin until QEMU has started and let the module's
# RI poll consume it. QEMU exits only when timeout terminates the PASS spin.
printf 'Z' | timeout 8 "$BIN/qemu-system-mcs251" -M stc32g144k246 -bios "$W/image.hex" -accel tcg \
  -icount shift=0,align=off,sleep=off -display none -monitor none -serial stdio \
  > "$W/serial.raw" 2> "$W/qemu.stderr"
QRC=${PIPESTATUS[1]}
set -e
...
PY
if [ "$QRC" -ne 124 ]; then
  echo "uart-echo-poll: expected timeout 124, got $QRC" >&2
  exit 1
fi
printf 'uart-echo-poll: PASS\n'
